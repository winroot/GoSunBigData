...
## Filename:    start-check-spring-dynrepo.sh
...
function check_spring_cloud()
{
    ...
    spring_pid=$(jps | grep "DynRepoApplication" | awk '{print $1}' | uniq)
    ...
    if [ -n "${spring_pid}" ];then
        ...
        sh ${BIN_DIR}/start-spring-cloud-dynrepo.sh
        echo "starting, please wait........" | tee -a $CHECK_LOG_FILE
        sleep 1m
        spring_pid_restart=$(jps | grep "DynRepoApplication" | awk '{print $1}' | uniq)
        if [ -z "${spring_pid_restart}" ];then
            ...
            sleep 1m
            spring_pid_retry=$(jps | grep "DynRepoApplication" | awk '{print $1}' | uniq)
