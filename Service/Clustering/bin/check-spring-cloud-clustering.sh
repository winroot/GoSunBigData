...
## Filename:    check-spring-cloud-clustering.sh
...
function check_spring_cloud()
{
    ...
    spring_pid=$(jps | grep "ClusteringApplication" | awk '{print $1}' | uniq)
    ...
    if [ -n "${spring_pid}" ];then
        ...
        sh ${BIN_DIR}/start-spring-cloud-clustering.sh
        echo "starting, please wait........" | tee -a $CHECK_LOG_FILE
        sleep 1m
        spring_pid_restart=$(jps | grep "ClusteringApplication" | awk '{print $1}' | uniq)
        if [ -z "${spring_pid_restart}" ];then
            ...
            sleep 1m
            spring_pid_retry=$(jps | grep "ClusteringApplication" | awk '{print $1}' | uniq)
