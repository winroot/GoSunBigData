...
## Filename:    start-spring-cloud-all
## Description: 在配置节点下启动spring cloud
## Author:      wujiaqi
## Created:     2017-5-5
################################################################################
...
LOG_FILE=$LOG_DIR/start-all-spring-cloud.log
...
#####################################################################
# 函数名: start_all_spring_cloud
# 描述: 启动spring cloud
# 参数: N/A
# 返回值: N/A
# 其他: N/A
#####################################################################
function start_all_spring_cloud()
{
    echo ""  | tee -a $LOG_FILE
    echo "**********************************************" | tee -a $LOG_FILE
    echo "" | tee -a $LOG_FILE
    ...
    do
        echo "${spring_host}节点下启动spring cloud................."  | tee  -a  $LOG_FILE
        ssh root@${spring_host}  "source /etc/profile;cd ${BIN_DIR};sh start-spring-cloud.sh"
        if [ $? -eq 0 ];then
            echo  -e '${spring_host}节点下启动spring cloud成功\n'
        else
            echo  -e '${spring_host}节点下启动spring cloud失败 \n'
        fi
    done
    echo "启动spring cloud完毕......"  | tee  -a  $LOG_FILE
}
