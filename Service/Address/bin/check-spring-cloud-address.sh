...
## Filename:    check-spring-cloud-address.sh
...
cd ..
CLUSTERING_DIR=`pwd`                               ### address 目录
LIB_CLUSTERING_DIR=${CLUSTERING_DIR}/lib              ### address lib
CONF_CLUSTERING_DIR=${CLUSTERING_DIR}/conf            ### address 配置文件目录
...
function check_spring_cloud()
{
    ...
    if [ -n "${spring_pid}" ];then
        ...
        sh ${BIN_DIR}/start-spring-cloud-address.sh
        echo "starting, please wait........" | tee -a $CHECK_LOG_FILE
        sleep 1m
        spring_pid_restart=$(jps | grep "FtpApplication" | awk '{print $1}' | uniq)
        if [ -z "${spring_pid_restart}" ];then
            ...
            sleep 1m
            spring_pid_retry=$(jps | grep "FtpApplication" | awk '{print $1}' | uniq)
