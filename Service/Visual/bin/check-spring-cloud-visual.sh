...
## Filename:    start-check-spring-visual.sh
...
#####################################################################
# 函数名: check_spring_cloud_visual
# 描述: 把脚本定时执行，spring_cloud 服务是否挂掉，如果挂掉则重启。
# 参数: N/A
# 返回值: N/A
# 其他: N/A
#####################################################################
function check_spring_cloud_visual()
{
    echo ""  | tee  -a  $CHECK_LOG_FILE
    echo "****************************************************"  | tee -a $CHECK_LOG_FILE
    echo "spring cloud procceding ing......................."  | tee  -a  $CHECK_LOG_FILE
    spring_pid=$(jps | grep "VisualApplication" | awk '{print $1}' | uniq)
    ...
    if [ -n "${spring_pid}" ];then
        ...
        sh ${BIN_DIR}/start-spring-cloud-visual.sh
        echo "starting, please wait........" | tee -a $CHECK_LOG_FILE
        sleep 1m
        spring_pid_restart=$(jps | grep "VisualApplication" | awk '{print $1}' | uniq)
        if [ -z "${spring_pid_restart}" ];then
            ...
            sleep 1m
            spring_pid_retry=$(jps | grep "VisualApplication" | awk '{print $1}' | uniq)
