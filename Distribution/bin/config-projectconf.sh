...
CONF_OBJECT_DIR=$OBJECT_DIR/conf                      ### 配置文件目录
CONF_FILE=$CONF_OBJECT_DIR/project-conf.properties    ### 项目配置文件

LOG_DIR=$OBJECT_DIR/logs                              ### log日志目录
LOG_FILE=$OBJECT_DIR/config-project.log               ### log日志目录

CLUSTER_DIR=$OBJECT_DIR/spark                         ### spark模块部署目录
FTP_DIR=$OBJECT_DIR/ftp                               ### ftp模块部署目录
SERVICE_DIR=$OBJECT_DIR/service                       ### service模块部署目录

CLUSTER_SCRPIT=$CLUSTER_DIR/bin/config-clusterconf.sh ### 一键配置spark脚本
...
function config_common_dubbo()
{
    ...
    echo "**********************************************" | tee -a $LOG_FILE
    echo "" | tee -a $LOG_FILE
    echo "配置conf/dubbo-hostnames.properties......"  | tee  -a  $LOG_FILE
    
    # 删除原本dubbo-hostnames.properties内容（从第一行开始的行）
    sed -i '1,$d' ${CONF_OBJECT_DIR}/dubbo-hostnames.properties
    ...
    do
        echo "${dubbo_host}" >> ${CONF_OBJECT_DIR}/dubbo-hostnames.properties
    done
...
function distribute_common()
{
    ...
    do
        ...
        rsync -rvl ${OBJECT_DIR}/bin   root@${hostname}:${OBJECT_DIR}  >/dev/null
        rsync -rvl ${OBJECT_DIR}/conf   root@${hostname}:${OBJECT_DIR} >/dev/null
