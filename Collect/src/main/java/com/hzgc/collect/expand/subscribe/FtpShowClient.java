 ...
 */
public class FtpShowClient extends ZookeeperClient {
    private static Logger LOG = Logger.getLogger(FtpShowClient.class);

    public FtpShowClient(int session_timeout, String zookeeperAddress, String path, boolean watcher) {
     ...
     * 创建人脸抓拍演示功能节点
     */
    public void createFtpShowZnode() {
        super.create();
    }

    /**
     * 获取人脸抓拍演示功能节点所有子节点(长期调用，故不自动创建连接，不关闭连接)
     */
    ...
    /**
     * 获取人脸抓拍演示功能子节点数据(长期调用，故不自动创建连接，不关闭连接)
     */
    ...
    /**
     * 删除人脸抓拍演示功能节点(长期调用，故不自动创建连接，不关闭连接)
     */
