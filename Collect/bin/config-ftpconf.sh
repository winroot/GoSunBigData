...
FTP_DATA_PATH=$(grep FTP_DataDir $OBJECT_DIR/conf/project-conf.properties | cut -d '=' -f2)
...
#####################################################################
# 函数名: config_collect
# 描述: 配置文件：ftp/conf/collect.properties
# 参数: N/A
# 返回值: N/A
# 其他: N/A
#####################################################################
function config_collect()
{
    echo ""  | tee -a $LOG_FILE
    echo "**********************************************" | tee -a $LOG_FILE
    echo "" | tee -a $LOG_FILE
    echo "配置ftp/conf/collect.properties......"  | tee  -a  $LOG_FILE
    ...
    sed -i "s#^thread.number=.*#thread.number=${THREAD_NUMBER}#g" ${CONF_FTP_DIR}/collect.properties
    ...
    sed -i "s#^zookeeper.address=.*#zookeeper.address=${zkpro}#g" ${CONF_FTP_DIR}/collect.properties

    SHARPNESS=`sed '/SharpNess/!d;s/.*=//' ${CONF_FILE} | tr -d '\r'`
    sed -i "s#^sharpness=.*#sharpness=${SHARPNESS}#g" ${CONF_FTP_DIR}/collect.properties

    FTP_SWITCH=`sed '/Ftp_switch/!d;s/.*=//' ${CONF_FILE} | tr -d '\r'`
    sed -i "s#^ftp.switch=.*#ftp.switch=${FTP_SWITCH}#g" ${CONF_FTP_DIR}/collect.properties
...
#####################################################################
# 函数名: config_kafka-producer
# 描述: 配置文件：ftp/conf/kafka-producer.properties
# 参数: N/A
# 返回值: N/A
# 其他: N/A
#####################################################################
function config_kafka-producer()
{
    echo ""  | tee -a $LOG_FILE
    echo "**********************************************" | tee -a $LOG_FILE
    echo "" | tee -a $LOG_FILE
    echo "配置ftp/conf/kafka-producer.properties......"  | tee  -a  $LOG_FILE
    ...
    # 替换kafka-producer.properties中：key=value（替换key字段的值value）
    sed -i "s#^bootstrap.servers=.*#bootstrap.servers=${kafkapro}#g" ${CONF_FTP_DIR}/kafka-producer.properties
