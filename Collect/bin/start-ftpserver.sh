...
BIN_DIR=`pwd`    ### bin目录
cd ..
DEPLOY_DIR=`pwd`             ### ftp目录
CONF_DIR=$DEPLOY_DIR/conf    ### 项目根目录
SERVER_NAME=`sed '/ftpserver.application.name/!d;s/.*=//' conf/collect.properties | tr -d '\r'` #获取服务名称
SERVER_PORT=`sed '/listener-port/!d;s/.*=//' conf/collect.properties | tr -d '\r'` #获取服务端口号
...
cd -
COMMON_JARS=`ls ${BIGDATA_SERVICE_DIR}/lib | grep .jar | awk '{print "'${BIGDATA_SERVICE_DIR}/lib'/"$0}'|tr "\n" ":"`
