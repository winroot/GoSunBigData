...
# 描述: index_es的子函数，替换create-dynamic-index.sh中的节点名
# 参数: N/A
# 返回值: N/A
# 其他: N/A
#####################################################################
...
{
    # 判断脚本是否存在，存在才执行
    if [ -f "${BIN_DIR}/create-dynamic-index.sh" ]; then
        ...
        sed -i "s#^${a1}.*${b1}#${replace1}#g" ${BIN_DIR}/create-dynamic-index.sh
        ...
        sed -i "s#^${a2}.*${b2}#${replace2}#g" ${BIN_DIR}/create-dynamic-index.sh
        ...
        sed -i "s#^\"number_of_shards\"#\"number_of_shards\":${Shards}#g" ${BIN_DIR}/create-dynamic-index.sh
        sed -i "s#^\"number_of_replicas\"#\"number_of_replicas\":${Replicas}#g" ${BIN_DIR}create-dynamic-index.sh
        sed -i "s#^\"max_result_window\"#\"max_result_window\":${Max_Result_Window}#g" ${BIN_DIR}/create-dynamic-index.sh


        sh ${BIN_DIR}/create-dynamic-index.sh
		if [ $? = 0 ];then
			echo "修改create-dynamic-index.sh成功并执行......"  | tee  -a  $LOG_FILE
		fi
    else
        echo "create-dynamic-index.sh不存在...."
    fi
}
...
# 描述: index_es的子函数，替换create-dynamicShow-index.sh中的节点名
# 参数: N/A
# 返回值: N/A
# 其他: N/A
#####################################################################
...
{
    # 判断脚本是否存在，存在才执行
    if [ -f "${BIN_DIR}/create-dynamicShow-index.sh" ]; then

        ### 替换create-dynamicShow-index.sh中的节点名，共三处
        # 要替换的节点名，如s106
        ES_IP=$(grep es_service_node ${CONF_FILE} | cut -d '=' -f2)
        ES_Host=$(cat /etc/hosts|grep "$ES_IP" | awk '{print $2}')
        ...
        replace1="curl -XDELETE '${ES_Host}:9200/dynamic?pretty'  -H 'Content-Type: application/json'"
        # ^表示以什么开头，.*a表示以a结尾。替换以a1开头、b1结尾匹配到的字符串为repalce1
        sed -i "s#^${a1}.*${b1}#${replace1}#g" ${BIN_DIR}/create-dynamicShow-index.sh

        ## 第二处
        a2="curl -XPUT '"
        b2="/dynamic?pretty' -H 'Content-Type: application/json' -d'"
        replace2="curl -XPUT '${ES_Host}:9200/dynamic?pretty' -H 'Content-Type: application/json' -d'"
        sed -i "s#^${a2}.*${b2}#${replace2}#g" ${BIN_DIR}/create-dynamicShow-index.sh

        ## 第三处
        a3="curl -XPUT '"
        b3="/dynamic/_settings' -d '{"
        replace3="curl -XPUT '${ES_Host}:9200/dynamic/_settings' -d '{"
        sed -i "s#^${a3}.*${b3}#${replace3}#g" ${BIN_DIR}/create-dynamicShow-index.sh



        sh ${BIN_DIR}/create-dynamicShow-index.sh
		if [ $? = 0 ];then
			echo "修改create-dynamicShow-index.sh成功并执行......"  | tee  -a  $LOG_FILE
		fi
    else
        echo "create-dynamicShow-index.sh不存在...."
    fi
}
